...
export default function Dashboard() {
  ...
  const cards = [
    { label: '등록 제품', value: stats.products, icon: Package, color: 'text-[#D84E0B]', bg: 'bg-[#D84E0B]/10' },
    { label: '접수 문의', value: stats.inquiries, icon: MessageCircle, color: 'text-[#8B5E3C]', bg: 'bg-[#8B5E3C]/10' },
  ...
  return (
    <div className="w-full">
      <div className="mb-8">
        <h2 className="font-serif-kr text-2xl font-bold text-[#2A211C]">대시보드</h2>
        <p className="text-[#a98c5b] mt-1">고르미 콘텐츠 현황을 한눈에 확인하세요.</p>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {cards.map((c) => (
          <div key={c.label} className="rounded-2xl bg-white border border-[#eadfce] p-5">
            <div className={`w-11 h-11 rounded-xl ${c.bg} flex items-center justify-center mb-4`}>
              <c.icon className={`w-5 h-5 ${c.color}`} />
            </div>
            <p className="text-2xl font-bold text-[#2A211C]">{loading ? '—' : c.value}</p>
            <p className="text-sm text-[#a98c5b] mt-1">{c.label}</p>
          </div>
        ))}
      </div>
