  ...
  return (
    <div className="w-full">
      <section className="w-full bg-[#2A211C] text-white overflow-hidden relative">
        <div className="absolute -top-24 -right-24 w-96 h-96 rounded-full bg-[#D84E0B]/25 blur-3xl" />
      ...
      <section className="w-full py-16 md:py-24 bg-[#FBF7F0]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <Reveal>
              <p className="text-[#D84E0B] font-semibold mb-3">창업 배경</p>
              <h2 className="font-serif-kr text-3xl md:text-4xl font-bold text-[#2A211C] leading-tight">
                전문가의 도구를<br />누구나 이해하도록
              </h2>
              <p className="mt-6 text-[#5a4d42] leading-relaxed">
              ...
              <Link
                to="/tray-builder"
                className="inline-flex items-center gap-2 mt-8 px-6 py-3.5 rounded-xl bg-[#D84E0B] text-white font-semibold hover:bg-[#b8420a] active:scale-95 transition-all"
              >
            ...
            <Reveal delay={0.15}>
              <div className="space-y-4">
                {TIMELINE.map((t, i) => (
                  <div key={i} className="flex gap-5 rounded-2xl bg-white border border-[#eadfce] p-6">
                    <div className="flex-shrink-0 w-16 text-center">
                      <span className="font-serif-kr text-[#D84E0B] font-bold">{t.year}</span>
                    </div>
                    <div>
                      <h3 className="font-serif-kr font-bold text-[#2A211C] mb-1">{t.title}</h3>
                      <p className="text-sm text-[#6b5d50] leading-relaxed">{t.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="w-full py-16 md:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal className="max-w-2xl mb-12">
            <p className="text-[#D84E0B] font-semibold mb-3">핵심 차별점 요약</p>
            <h2 className="font-serif-kr text-3xl md:text-4xl font-bold text-[#2A211C]">고르미가 지키는 것</h2>
          </Reveal>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {VALUES.map((v, i) => (
              <Reveal key={v.title} delay={i * 0.1}>
                <div className="h-full rounded-3xl bg-[#F7F1E8]/70 border border-[#e7dcc9] p-8">
                  <div className="w-14 h-14 rounded-2xl bg-white flex items-center justify-center mb-5 shadow-sm">
                    <v.icon className="w-7 h-7 text-[#D84E0B]" />
                  </div>
                  <h3 className="font-serif-kr text-xl font-bold text-[#2A211C] mb-2">{v.title}</h3>
                  <p className="text-[#6b5d50] leading-relaxed">{v.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
          <Reveal delay={0.2}>
            <div className="mt-12 rounded-3xl border border-[#eadfce] p-8 flex flex-col md:flex-row items-start gap-5">
              <Palette className="w-8 h-8 text-[#D84E0B] flex-shrink-0" />
              <div>
                <h3 className="font-serif-kr font-bold text-[#2A211C] mb-2">브랜드 컬러</h3>
                <p className="text-[#6b5d50] leading-relaxed">
                  따뜻한 노을을 닮은 오렌지와 넉넉한 여백의 아이보리. 공예 감성과 신뢰감을 함께 담았습니다.
                </p>
                <div className="mt-4 flex gap-3">
                  <div className="flex items-center gap-2">
                    <span className="w-7 h-7 rounded-full" style={{ backgroundColor: '#D84E0B' }} />
                    <span className="text-sm text-[#6b5d50]">#D84E0B</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="w-7 h-7 rounded-full border border-[#e0d3bd]" style={{ backgroundColor: '#F7F1E8' }} />
                    <span className="text-sm text-[#6b5d50]">#F7F1E8</span>
                  </div>
                </div>
              </div>
            </div>
