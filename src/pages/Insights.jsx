  ...
  return (
    <div className="w-full bg-[#F2F1EE]">
      <section className="w-full overflow-hidden relative">
        <div className="absolute -top-20 -right-16 w-80 h-80 rounded-full bg-[#A97C3F]/10 blur-3xl" />
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-8 md:pt-16">
          <Reveal className="max-w-3xl">
            ...
            <h1 className="font-serif-kr text-3xl md:text-5xl font-bold text-[#1E1B18] mt-5">도구를 고르는 관점</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[0, 1, 2].map((i) => (
                <div key={i} className="rounded-3xl overflow-hidden bg-white border border-[#DAD6CC] animate-pulse">
                  <div className="aspect-[16/10] bg-[#E6E3DC]" />
                  <div className="p-6 space-y-3">
                    <div className="h-3 bg-[#E6E3DC] rounded w-1/3" />
                    <div className="h-5 bg-[#E6E3DC] rounded w-2/3" />
                    <div className="h-4 bg-[#E6E3DC] rounded" />
                  </div>
                </div>
              ))}
            </div>
              ...
              {insights.map((it, i) => (
                <Reveal key={it.id} delay={(i % 3) * 0.08}>
                  <Link to={`/insights/${it.slug}`} className="group block h-full">
                    <div className="h-full rounded-3xl overflow-hidden bg-white border border-[#DAD6CC] hover:-translate-y-1 hover:shadow-lg transition-all">
                      ...
                      <div className="p-6">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className="text-xs text-[#948A76] flex items-center gap-1"><Clock className="w-3 h-3" /> {it.readMinutes || 3}분</span>
                        </div>
                        <h3 className="font-serif-kr text-lg font-bold text-[#1E1B18] mb-2 line-clamp-2">{it.title}</h3>
                        <p className="text-sm text-[#5C574C] leading-relaxed line-clamp-2 mb-3">{it.excerpt}</p>
                        <span className="text-sm font-semibold text-[#A97C3F] flex items-center gap-1 group-hover:gap-2 transition-all">
